...
class main_module
{
	...
	function main($id, $mode)
	{
		...
		global $phpbb_root_path, $phpbb_admin_path, $phpEx, $phpbb_log;
		...
		$template->assign_vars(array(
			...
			'MEETING_VERSION'	=> $language->lang('MEETING_VERSION', $config['meeting_version']),
		...
		switch($mode)
		{
			case 'config':
				...
				if ($submit)
				{
					...
					$phpbb_log->add('admin', $user->data['user_id'], $user->ip, 'MEETING_LOG_CONFIG');
				...
				$s_meeting_sign_perm .= '<option value="2">' . $language->lang('MEETING_SIGN_OTHER_P3') . '</option>';
				$s_meeting_sign_perm .= '<option value="3">' . $language->lang('MEETING_SIGN_OTHER_P4') . '</option>';
			...
			case 'manage':
				...
				if ($submit)
				{
					...
					if (!$meeting_time)
					{
						trigger_error($language->lang('MEETING_TIME_WRONG'), E_USER_WARNING);
					}

					$meeting_end = meeting_date_save($e_date, $e_time);
					if (!$meeting_end)
					{
						trigger_error($language->lang('MEETING_END_WRONG'), E_USER_WARNING);
					}

					$meeting_until = meeting_date_save($u_date, $u_time);
					if (!$meeting_time)
					{
						trigger_error($language->lang('MEETING_UNTIL_WRONG'), E_USER_WARNING);
					}
					...
					$phpbb_log->add('admin', $user->data['user_id'], $user->ip, $log_title, false, array($meeting_subject));

					$meeting_save_text = ($m_id) ? $language->lang('MEETING_DATA_UPDATED') : $language->lang('MEETING_DATA_STORED');
				...
				if ($m_id || $mode == 'add')
				{
					...
					$meeting_by_user			= $language->lang('MEETING_CREATE_BY', append_sid("{$phpbb_root_path}memberlist.$phpEx", "mode=viewprofile&amp;u=" . $user->data['user_id']), $user->data['username']);
					...
					if ($mode == 'manage' && $m_id)
					{
						...
						$meeting_by_user		= $language->lang('MEETING_CREATE_BY', $meeting_by_user_id, $meeting_by_username);
						$meeting_edit_by_user	= $language->lang('MEETING_EDIT_BY', $meeting_edit_by_user_id, $meeting_edit_by_username);
					}
				...
				if ($mode == 'manage' && !$m_id)
				{
					...
					$sort_by_field .= '<option value="meeting_time">'	.	$language->lang('TIME')	.	'</option>';
					$sort_by_field .= '<option value="meeting_until">'	.	$language->lang('MEETING_UNTIL')	.	'</option>';
