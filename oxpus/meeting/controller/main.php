...
class main
{
	...
	*/
	...
	public function handle($view = '')
	{
		...
		$page_start = max($start, 0) * $this->config['topics_per_page'];
		$start = $page_start;
		...
		$basic_link			= $this->helper->route('oxpus_meeting_controller', array_merge($basic_values, array('mode' => $mode)));
		$basic_link_smode	= $this->helper->route('oxpus_meeting_controller', $basic_values);
		$basic_link_page	= $this->helper->route('oxpus_meeting_controller', array_merge($basic_values_page, array('mode' => $mode)));
		$basic_link_start	= $this->helper->route('oxpus_meeting_controller', array_merge($basic_values_start, array('mode' => 'manage', 'start' => $start)));
		...
		switch ($mode)
		{
			case 'mail':
				...
				$m_nav_name	= $this->language->lang('MEETING_DETAIL');
			break;
		...
		$meeting_nav_link[] = array('m_link'	=> $basic_link_start, 'm_name'	=> $this->language->lang('MEETING'));
		...
		$this->template->assign_vars(array(
			...
			'MEETING_VERSION'	=> $this->language->lang('MEETING_VERSION'),
		...
		*/
		switch($mode)
		{
			case 'mail':
					...
					if ($meeting_subject && $meeting_mail_subject && $meeting_mail_text )
					{
						...
						while ($row = $this->db->sql_fetchrow($result))
						{
							...
							$messenger->assign_vars(array(
								...
								'U_MEETING_MAIL_LINK'	=> generate_board_url(true) . $this->helper->route('oxpus_meeting_controller', array('mode' => 'detail', 'm_id' => $m_id)),
							));
				...
				$this->template->assign_vars(array(
					'L_MEETING'			=> $this->language->lang('MEETING_VIEWLIST'),
			...
			case 'edit':
				...
				if ($submit)
				{
					...
					if (!$meeting_time)
					{
						trigger_error($this->language->lang('MEETING_TIME_WRONG'), E_USER_WARNING);
					}

					$meeting_end = meeting_date_save($e_date, $e_time);
					if (!$meeting_end)
					{
						trigger_error($this->language->lang('MEETING_END_WRONG'), E_USER_WARNING);
					}

					$meeting_until = meeting_date_save($u_date, $u_time);
					if (!$meeting_time)
					{
						trigger_error($this->language->lang('MEETING_UNTIL_WRONG'), E_USER_WARNING);
					}
					...
					$meeting_save_text = ($m_id) ? $this->language->lang('MEETING_DATA_UPDATED') : $this->language->lang('MEETING_DATA_STORED');
					$message = $meeting_save_text . '<br /><br />' . $this->language->lang('CLICKMEETINGBACK', '<a href="' . $basic_link_smode . '&amp;mode=manage">', '</a>');
				...
				$s_meeting_usergroup .= '<option value="-1"' . (($mode == 'add') ? 'selected="selected"' : '') . '>' . $this->language->lang('MEETING_ALL_USERS') . '</option>';
				...
				$meeting_by_user			= $this->language->lang('MEETING_CREATE_BY', append_sid($this->root_path . 'memberlist.' . $this->php_ext, "mode=viewprofile&amp;u=" . $this->user->data['user_id']), $this->user->data['username']);
				...
				if ($m_id)
				{
					...
					$meeting_by_user		= $this->language->lang('MEETING_CREATE_BY', $meeting_by_user_id, $meeting_by_username);
					$meeting_edit_by_user	= $this->language->lang('MEETING_EDIT_BY', $meeting_edit_by_user_id, $meeting_edit_by_username);
				}
			...
			case 'delete':
				...
				if (!$confirm)
				{
					...
					$this->template->assign_vars(array(
						'MESSAGE_TITLE'		=> $this->language->lang('MEETING_DELETE'),
						'MESSAGE_TEXT'		=> $this->language->lang('MEETING_DELETE_EXPLAIN'),
			...
			case 'detail':
				...
				if ($action == 'edit_comment')
				{
					...
					if ($c_user_id && $c_id)
					{
						...
						$comment_mode		= $this->language->lang('MEETING_EDIT_COMMENT');
					...
					else
					{
						...
						$comment_mode		= $this->language->lang('MEETING_POST_COMMENT');
				...
				if ($action == 'delete_comment')
				{
					if (!$confirm)
					{
						...
						$this->template->assign_vars(array(
							'MESSAGE_TITLE'		=> $this->language->lang('DELETE'),
							'MESSAGE_TEXT'		=> $this->language->lang('CONFIRM_OPERATION'),
				...
				if ($action == 'sign_on')
				{
					...
					{
						trigger_error($this->language->lang('MEETING_REMAIN_GUEST_TEXT', $meeting_guests, $remain_guests), E_USER_WARNING);
					}
					...
					if (!$submit_check)
					{
						...
						if ($meeting_sure <> 0)
						{
							$subject = $this->language->lang('MEETING_JOIN_MESSAGE');
							$message = $this->language->lang('MEETING_JOIN_USER', $signed_username, $meeting_subject);
						}
						else
						{
							$subject = $this->language->lang('MEETING_UNWILL_MESSAGE');
							$message = $this->language->lang('MEETING_UNWILL_USER', $signed_username, $meeting_subject);
						}
					}
					else
					{
						...
						if ($meeting_sure <> 0)
						{
							$subject = $this->language->lang('MEETING_CHANGE_MESSAGE');
							$message = $this->language->lang('MEETING_CHANGE_USER', $signed_username, $meeting_subject);
						}
						else
						{
							$subject = $this->language->lang('MEETING_UNWILL_MESSAGE');
				...
				if ($action == 'sign_off' && $m_id)
				{
					if (!$confirm)
					{
						...
						$this->template->assign_vars(array(
							'MESSAGE_TITLE'	=> $this->language->lang('MEETING_SIGN_OFF'),
							'MESSAGE_TEXT'	=> ($user_id <> $this->user->data['user_id']) ? $this->language->lang('MEETING_SIGN_OFF_EXPLAIN_OTHER') : $this->language->lang('MEETING_SIGN_OFF_EXPLAIN'),
					...
					if ($this->config['meeting_notify'] || $meeting_notify)
					{
						$subject = $this->language->lang('MEETING_UNJOIN_MESSAGE');
						$sign_off_string = ($meeting_user <> $this->user->data['user_id']) ? $this->language->lang('MEETING_UNJOIN_OTHER_USER') : $this->language->lang('MEETING_UNJOIN_USER');
						$message = sprintf($sign_off_string, $username, $meeting_subject);
				...
				while ($row = $this->db->sql_fetchrow($result))
				{
					...
					$meeting_user .= ($row['meeting_sure']) ? (($row['meeting_sure'] == 100) ? $this->language->lang('MEETING_YES_SIGNON') : $row['meeting_sure'].'%') : $this->language->lang('MEETING_NO_SIGNON');

					if ($guests)
					{
						if ($meeting_guest_names)
						{
							$meeting_guests_text = ($guests == 1) ? $this->language->lang('MEETING_USER_GUEST_POPUP', $m_id, $row['user_id']) : $this->language->lang('MEETING_USER_GUESTS_POPUP', $m_id, $row['user_id'], $guests);
							$guest_popup = str_replace("&amp;", "&", $basic_link_smode) . "&mode=popup&m_id=$m_id&u=$signed_on_user";
						}
						else
						{
							$meeting_guests_text = ($guests == 1) ? $this->language->lang('MEETING_USER_GUEST') : $this->language->lang('MEETING_USER_GUESTS', $guests);
							$guest_popup = '';
						}
				...
				{
					$remain_guests = $meeting_guest_overall - $total_guests;
					$guest_places .= ($meeting_guest_overall == 1) ? $this->language->lang('MEETING_OVERALL_GUEST_PLACES_ONE') : $this->language->lang('MEETING_OVERALL_GUEST_PLACES', $meeting_guest_overall);
				}

				if ($meeting_guest_single)
				{
					$guest_places .= ($meeting_guest_single == 1) ? $this->language->lang('MEETING_SINGLE_GUEST_PLACES_ONE') : $this->language->lang('MEETING_SINGLE_GUEST_PLACES', $meeting_guest_single);
					$remain_guests = $meeting_guest_single;
					if ($meeting_guest_overall && $remain_guests > ($meeting_guest_overall - $total_guests))
					{
						$remain_guests = $meeting_guest_overall - $total_guests;
				...
				$meeting_closed_string = ($meeting_tmp_time < time()) ? '[ ' . $this->language->lang('MEETING_CLOSED') . ' ]' : (($meeting_until < time()) ? '[ ' . $this->language->lang('MEETING_NO_PERIOD') . ' ]' : '');
				...
				{
					$remain_guest_places = ($remain_guests == 1) ? $this->language->lang('MEETING_REMAIN_GUEST_PLACES_ONE') : $this->language->lang('MEETING_REMAIN_GUEST_PLACES', $remain_guests);
					...
					{
						$s_remain_guests = $this->language->lang('MEETING_INVITE_GUESTS') . '<select name="meeting_guests">';
						for ($i = 0; $i <= $remain_guests; $i++)
						{
							$s_remain_guests .= '<option value="' . $i . '">' . $i . '</option>';
						}
						$s_remain_guests .= '</select>&nbsp;' . $this->language->lang('MEETING_GUESTS');
				...
				if ($total_guests)
				{
					$total_guests_text = ($total_guests == 1) ? $this->language->lang('MEETING_USER_GUEST') : $this->language->lang('MEETING_USER_GUESTS', $total_guests);
				}
				else
				{
					$total_guests_text = '';
				...
				$meeting_user = ($meeting_total_user_ids == 0) ? $this->language->lang('MEETING_NO_USER') : '<strong>' . ($meeting_total_user_ids + $total_guests) . ' ' . $this->language->lang('MEETING_USER_JOINS') . '</strong> (' . $this->language->lang('MEETING_YES_SIGNONS') . $meeting_user_ids . $total_guests_text . '):<br />' . substr($meeting_user, 0, strlen($meeting_user)-1);

				if ($remain_guests)
				{
					$meeting_free_guests = ($remain_guests == 1) ? $this->language->lang('MEETING_USER_GUEST') : $this->language->lang('MEETING_USER_GUESTS', $remain_guests);
				}
				else
				{
					$meeting_free_guests = '';
				...
				switch ($meeting_closed)
				{
					case 0:
						if ($signed_on)
						{
							$s_signed_on_edit = $this->language->lang('MEETING_SIGN_EDIT');
							$s_signed_off = $this->language->lang('MEETING_SIGN_OFF');
						}
						else if ($meeting_free_places)
						{
							$s_signed_on_edit = $this->language->lang('MEETING_SIGN_ON');
							$s_signed_off = '';
						}
					break;
				...
				if ($meeting_free_places != 0 || ($meeting_free_places == 0 && $signed_on == true))
				{
					if ($meeting_closed == 0 || ($meeting_closed == 1 && $signed_on == true))
					{
						...
						{
							$meeting_sure_user .= '<option value="' . $i . '">' . (($i == 0) ? $this->language->lang('MEETING_NO_SIGNON') : $i . '%').'</option>';
						}
						$meeting_sure_user .= '<option value="100" selected="selected">' . $this->language->lang('MEETING_YES_SIGNON') . '</option>';
				...
				while ($row = $this->db->sql_fetchrow($result))
				{
					...
					{
						$temp_link_1 = '<a href="' . $basic_link_smode . "&amp;mode=detail&amp;action=delete_comment&amp;m_id=$m_id&amp;c_user_id=$comment_user&amp;c_id=$comment_id" . '">' . $this->language->lang('DELETE') . '</a>';
					}
					else
					{
						$temp_link_1 = '';
					...
					{
						$temp_link_2 = '<a href="' . $basic_link_smode . "&amp;mode=detail&amp;action=edit_comment&amp;m_id=$m_id&amp;c_user_id=$comment_user&amp;c_id=$comment_id" . '">' . $this->language->lang('EDIT_POST') . '</a>';
					}
					else
					{
						$temp_link_2 = '';
					...
					{
						$temp_link_3 = '<a href="' . $basic_link_smode . "&amp;mode=detail&amp;action=approve_comment&amp;m_id=$m_id&amp;c_id=$comment_id" . '"><strong>' . $this->language->lang('POST_UNAPPROVED') . '</strong></a>';
					}
					else if ($row['approve'])
					{
			...
			default:
				...
				$this->template->assign_vars(array(
					'MODULE_NAME'		=> $this->language->lang('MEETING_MANAGE'),

					'L_MEETING_TIME'	=> $this->language->lang('TIME'),
				));

				// Create the sort and filter fields
				$sort_by_field = '<select name="sort_field">';
				$sort_by_field .= '<option value="meeting_subject">'	.	$this->language->lang('MEETING_SUBJECT')	.	'</option>';
				$sort_by_field .= '<option value="meeting_time">'	.	$this->language->lang('TIME')	.	'</option>';
				$sort_by_field .= '<option value="meeting_until">'	.	$this->language->lang('MEETING_UNTIL')	.	'</option>';
				$sort_by_field .= '<option value="meeting_location">'	.	$this->language->lang('MEETING_LOCATION')	.	'</option>';
				...
				$sort_by_order .= '<option value="ASC">' . $this->language->lang('MEETING_SORT_ASC') . '</option>';
				$sort_by_order .= '<option value="DESC">' . $this->language->lang('MEETING_SORT_DESC') . '</option>';
				...
				$filter_by_field .= '<option value="meeting_subject">' . $this->language->lang('MEETING_SUBJECT') . '</option>';
				$filter_by_field .= '<option value="meeting_location">' . $this->language->lang('MEETING_LOCATION') . '</option>';
				...
				if ($view == 'cal')
				{
					...
					for ($i = 1; $i < 13; $i++)
					{
						...
						$s_select_cal_month .= '<option value="' . $month . '"' . $selected . '>' . $this->language->lang('MEETING_MONTH_TEXT_' . $i) . '</option>';
					}
				...
				if ($total_meetings && $view != 'cal')
				{
					...
					for($i = 0; $i < sizeof($meetingrow); $i++)
					{
						...
						$meeting_closed		= ($meeting_check_time < time()) ? $this->language->lang('YES') : (($meeting_check_until < time()) ? $this->language->lang('MEETING_NO_PERIOD') : $this->language->lang('NO'));
						...
						$meeting_users .= $this->language->lang('MEETING_YES_SIGNONS') . ': ' . ((isset($yes_meet_u[$meeting_id])) ? $yes_meet_u[$meeting_id] : 0);
						$meeting_users .= ((isset($yes_meet_g[$meeting_id])) ? ' (' . $yes_meet_g[$meeting_id] . ')' : '');
						$meeting_users .= '<br />' . $this->language->lang('MEETING_NO_SIGNONS') . ': ' . ((isset($no_meet_u[$meeting_id])) ? $no_meet_u[$meeting_id] : 0);
						$meeting_users .= ((isset($no_meet_g[$meeting_id])) ? ' (' . $no_meet_g[$meeting_id] . ')' : '');
						$meeting_users .= '<br />' . $this->language->lang('MEETING_MAYBE_SIGNONS') . ': ' . ((isset($maybe_meet_u[$meeting_id])) ? $maybe_meet_u[$meeting_id] : 0);
					...
					if ($total_meetings > $per_page)
					{
						...
						$this->template->assign_vars(array(
							...
							'TOTAL_MEETINGS'	=> $this->language->lang('MEETING_TOTALS', $total_meetings),
						));
					}
				}
				else if ($view == 'cal')
				{
					...
					for ($i = 0; $i < 7; $i++)
					{
						if ($this->config['meeting_first_weekday'] == 'm')
						{
							...
							if ($day == 7)
							{
								$day_title = $this->language->lang('MEETING_CAL_DAY_0');
							}
							else
							{
								$day_title = $this->language->lang('MEETING_CAL_DAY_' . $day);
							}
						}
						else
						{
							$day_title = $this->language->lang('MEETING_CAL_DAY_' . $i);
						}
				...
				if ($view == 'cal')
				{
					...
					$meeting_view = $this->language->lang('MEETING');
				}
				else
				{
					$back_link = str_replace("&amp;view=&amp;", "&amp;view=cal&amp;", $basic_link_start);
					$meeting_view = $this->language->lang('MEETING_CALENDAR');
				}
